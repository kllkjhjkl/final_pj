...
def get_3dgs_train_test_split(datadir, eval_mode=True, test_camera_indices=None):
    ...
    if test_camera_indices is not None:
        ...
        total_images = len([f for f in os.listdir(images_folder) if f.lower().endswith(('.png', '.jpg', '.jpeg'))])
        all_indices = set(range(total_images))
        i_test = test_camera_indices
        i_train = list(all_indices - set(i_test))
        return i_train, i_test
    
    # Method 2: Follow 3DGS logic exactly
    test_cam_names_list = []
    
    if eval_mode:
        # Check if this is a 360 dataset
        llffhold = None
        if "360" in datadir:
            llffhold = 8
        ...
        if llffhold:
            print("------------LLFF HOLD-------------")
            # Get all image names and sort them
            images_folder = os.path.join(datadir, 'images')
            if os.path.exists(images_folder):
                cam_names = [f for f in os.listdir(images_folder) if f.lower().endswith(('.png', '.jpg', '.jpeg'))]
                cam_names = sorted(cam_names)
                test_cam_names_list = [name for idx, name in enumerate(cam_names) if idx % llffhold == 0]
            else:
                # Fallback: use indices directly
                total_images = 100  # Default assumption
                test_cam_names_list = [f"{idx:03d}" for idx in range(0, total_images, llffhold)]
        else:
            # Try to read from sparse/0/test.txt
            test_file_path = os.path.join(datadir, "sparse", "0", "test.txt")
            if os.path.exists(test_file_path):
                with open(test_file_path, 'r') as file:
                    test_cam_names_list = [line.strip() for line in file]
                print(f"Loaded test camera names from {test_file_path}: {test_cam_names_list}")
    else:
        test_cam_names_list = []  # No test set in training mode
    ...
    if os.path.exists(images_folder):
        ...
        # Create mapping from image name to index
        name_to_index = {name: idx for idx, name in enumerate(all_image_files)}
        
        # Convert test camera names to indices
        i_test = []
        for test_name in test_cam_names_list:
            ...
            if test_name in name_to_index:
                ...
                # Try different extensions or formats
        ...
        # Filter out invalid indices and convert test camera names to indices
        i_test = []
        for test_name in test_cam_names_list:
            # Handle different naming conventions
            found = False
        ...
        print(f"Test camera names: {test_cam_names_list}")
        print(f"Test indices: {i_test}")
        
        return i_train, i_test
    
    else:
        # Fallback if images folder not found
        print(f"Warning: Images folder not found at {images_folder}")
        print("Using fallback logic: every 8th image as test")
        total_images = 100  # Default assumption
        i_test = list(range(0, total_images, 8))
        all_indices = set(range(total_images))
        i_train = list(all_indices - set(i_test))
        return i_train, i_test
